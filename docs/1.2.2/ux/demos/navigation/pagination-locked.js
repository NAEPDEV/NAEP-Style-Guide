(function() {
  ...
  PageController = (function() {
    function PageController(pageParent, myId) {
      var $navigator, checkEnabled, idx, j, k, l, len, margin, mkHandler, navFn, pageHeight, ref, ref1, tb;
      ...
      this._setNextBackButtonEnabled = bind(this._setNextBackButtonEnabled, this);
      ...
      $navigator.append($('<button class="btn-page-previous" title="Previous Page. Goes back one page."></button>'));
      for (idx = j = 1, ref = this.pages.length; 1 <= ref ? j <= ref : j >= ref; idx = 1 <= ref ? ++j : --j) {
        $navigator.append($("<button id='btn-page-" + idx + "' class='btn-page-num' title='Page Number. Use these to move between pages.'>" + idx + "</button>"));
      }
      $navigator.append($("<button class='btn-page-next' title='Next Page. Moves to the next page.'></button>"));
      this.disablePage(3);
      checkEnabled = (function(_this) {
        return function(evt) {
          return !(_this.turnDisabled || $(evt.target).attr('disabled') || $(evt.currentTarget).attr('disabled'));
        };
      })(this);

      $('#btn-enable-page').click((function(_this) {
        return function(evt) {
          _this.enablePage(3);
      ...
      $navigator.find('.btn-page-previous').click((function(_this) {
        return function(evt) {
          if (checkEnabled(evt)) {
      ...
      this.pageParent.find('.btn-page-next').click((function(_this) {
        return function(evt) {
          if (checkEnabled(evt)) {
      ...
      for (idx = k = 0, ref1 = this.pages.length; 0 <= ref1 ? k < ref1 : k > ref1; idx = 0 <= ref1 ? ++k : --k) {
        this.pageParent.find("\#btn-page-" + (idx + 1)).click(navFn(idx));
      }
      $navigator.bind('keydown', (function(_this) {
        return function(evt) {
          var $btn;
          switch (evt.keyCode) {
            case 37:
      ...
      document.addEventListener('updatePageButtons', (function(_this) {
        return function(evt) {
          var state = evt.detail.state;
          if (state == 'disable') {
            _this.turnDisabled = true;
            return _this.pageParent.find('.page-nav button').each(function() {
            ...
            for (pageNum = l = 1, ref2 = _this.pages.length; 1 <= ref2 ? l <= ref2 : l >= ref2; pageNum = 1 <= ref2 ? ++l : --l) {
              if (!_this.__disabledPages[pageNum]) {
                _this.pageParent.find("#btn-page-" + pageNum).removeAttr('disabled');
              }
            }
    ...
    PageController.prototype.getPageScale = function() {
      ...
      getScale = function(el) {
        ...
        if (style.transformOrigin !== "0px 0px") {
          console.warn(el.nodeName + "#" + el.id + " has a transform-origin property of " + style.transformOrigin + ". Values other than '0px 0px' may cause bugs in page button placement.");
        }
        ...
        if ((matches != null) && matches[1] === matches[2]) {
          ...
          console.warn(el.nodeName + "#" + el.id + " has a CSS transform that doesn't look like a 2D scale transform. This may cause bugs in page button placement.");
          return 1.0;
        }
      };
    ...
    PageController.prototype.positionNavigator = function() {
      ...
      return setTop = (function(_this) {
        ...
        ypos = (pageHeight / 2) - ($navigator.outerHeight() / 2) + _this.pageParent.offset().top;
    ...
    PageController.prototype._setNextBackButtonEnabled = function() {
      ...
      for (idx = k = ref1 = this.pages.length; ref1 <= 1 ? k <= 1 : k >= 1; idx = ref1 <= 1 ? ++k : --k) {
        if (!this.pageParent.find("#btn-page-" + idx).attr('disabled')) {
          break;
        }
      }
    ...
    PageController.prototype.enablePage = function(pageNum) {
      ...
      this.pageParent.find("#btn-page-" + pageNum).removeClass('locked').removeAttr('disabled');
    ...
    PageController.prototype.disablePage = function(pageNum) {
      ...
      pageNum = parseInt(pageNum, 10);
      this.pageParent.find("#btn-page-" + pageNum).attr('disabled', true).addClass('locked');
      if (this.activePage === pageNum - 1) {
        idx = null;
        goBack = true;
        if (pageNum < this.pages.length) {
          for (idx = j = ref = pageNum + 1, ref1 = this.pages.length; ref <= ref1 ? j <= ref1 : j >= ref1; idx = ref <= ref1 ? ++j : --j) {
        ...
        if (goBack && pageNum > 1) {
          for (idx = k = ref2 = pageNum - 1; ref2 <= 1 ? k <= 1 : k >= 1; idx = ref2 <= 1 ? ++k : --k) {
            if (!this.__disabledPages[idx]) {
              this.moveTo(idx - 1, 'auto', false);
              break;
            }
    ...
    PageController.prototype.moveTo = function(idx, method, quickTurn) {
      ...
      oldP = this.activePage != null ? this.myTabTitle + "page-" + (this.activePage + 1) : null;
      newP = this.myTabTitle + "page-" + (idx + 1);
      if (this.activePage != null) {
        document.dispatchEvent(new CustomEvent('beforePageTurn', {
          detail: {
            from: oldP,
            to: newP
          }
      ...
      this.pageParent.find('.btn-page-num').removeClass('page-active').removeAttr('tabindex');
      this.pageParent.find("#btn-page-" + (idx + 1)).addClass('page-active').attr('tabindex', -1);
      if (this.activePage != null) {
        distance = idx > this.activePage ? idx - this.activePage : this.activePage - idx;
      } else {
        distance = 0;
      }
      this.activePage = idx;
      ...
      fn = (function(_this) {
        return function() {
          return document.dispatchEvent(new CustomEvent('afterPageTurn', {
            detail: {
              from: oldP,
              to: newP
            }
    ...
    PageController.prototype.next = function(method) {
      ...
      this.__disabledPages || (this.__disabledPages = {});
      for (idx = j = ref = this.activePage + 2, ref1 = this.pages.length; ref <= ref1 ? j <= ref1 : j >= ref1; idx = ref <= ref1 ? ++j : --j) {
        if (!this.__disabledPages[idx]) {
    ...
    PageController.prototype.previous = function(method) {
      ...
      this.__disabledPages || (this.__disabledPages = {});
      for (idx = j = ref = this.activePage; ref <= 1 ? j <= 1 : j >= 1; idx = ref <= 1 ? ++j : --j) {
        if (!this.__disabledPages[idx]) {
          this.moveTo(idx - 1, method);
          return;
        }
