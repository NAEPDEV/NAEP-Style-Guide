...
function answerItem(target) {
  ...
  compsAnswered = $(".childItem.selected")
    .find(".response-area")
    .filter(function () {
...
function showTabTray() {
  if ($(".itemTab").length) {
    var leftBtnWidth = parseInt(
        $(".btn-tab-mover-wrapper:first-of-type").outerWidth()
      ),
...
function changeBtnState() {
  if ($(".itemTab").length) {
    var currentMargin = parseInt($(".itemTab").css("margin-left")),
      leftBtnWidth = parseInt(
        $(".btn-tab-mover-wrapper:first-of-type").outerWidth()
      ),
      rightBtnOffset = parseInt(
        $(".btn-tab-mover-wrapper:last-of-type").offset().left
      ),
...
function showActiveTab() {
  var currentMargin = parseInt($(".itemTab").css("margin-left")),
    leftBtnOffset = parseInt(
      $(".btn-tab-mover-wrapper:first-of-type").offset().left
    ),
    leftBtnWidth = parseInt(
      $(".btn-tab-mover-wrapper:first-of-type").outerWidth()
    ),
  ...
  ItemTabController = (function () {
    function ItemTabController(arg) {
      ...
      if (typeof scratchwork == "function") {
        ...
        heightEms = height / 24;
        this.element.append(
          '<canvas class="scratchwork-canvas" id="sw' +
            this.title +
            '" width="1416" height="' +
            height +
            '" style="height: ' +
